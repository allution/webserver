...
exports.getGame = function(gameId, callback) {
    ...
    query('SELECT games.*, game_hashes.hash FROM games games LEFT JOIN game_hashes game_hashes ON games.id = game_hashes.game_id WHERE id = $1 AND ended = TRUE;', [gameId], function(err, result) {
        if (err) return callback(err);
